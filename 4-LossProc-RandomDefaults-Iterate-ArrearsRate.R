# ======== Script 2: Loss Optimisation Procedure across Arrears Rate
...
it.vec <- round(seq(0,1,length.out = 9),3)
it.max <- NROW(it.vec) # number of iterations
given.filename <- "4-Data-RandomDefaults-ArrearsRate"
...
### no arrears loss rate specified since this script iterates across each value given in it.vec
...
for (it in 1:it.max) {
  ...
  # Iteration Variable
  Arrears.LossRate <- it.vec[it]
  it.var <- Arrears.LossRate # for automation purposes when creating dat.EL at the end of each loop
...
port.labels <- sapply(1:it.max,function(i)paste("r_A=", it.vec[i],sep=''))
legend.title <- bquote(Arrears~Loss~Rate~italic(r)[italic(A)])
