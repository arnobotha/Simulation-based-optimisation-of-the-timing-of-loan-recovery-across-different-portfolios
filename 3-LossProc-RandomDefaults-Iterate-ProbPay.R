# ======== Script 2: Loss Optimisation Procedure across Probabilities of Payment
...
it.vec <- round(c(0, 0.4,0.65, seq(0.75,1,length.out = 9)),2)
it.max <- NROW(it.vec) # number of iterations
given.filename <- "3-Data-RandomDefaults-ProbPay"
...
for (it in 1:it.max) {
  ...
  # Iteration Variable
  ProbPay <- it.vec[it]
  it.var <- ProbPay # for automation purposes when creating dat.EL at the end of each loop
...
port.labels <- sapply(1:it.max,function(i)paste("b=", it.vec[i],sep=''))
legend.title <- bquote(Probability~of~Payment~italic(b))
col.v <- rev(brewer.pal(12, "Paired"))
