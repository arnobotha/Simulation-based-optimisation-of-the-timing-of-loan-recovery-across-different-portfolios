...
# a) Portfolio simulated using: EPISODIC DEFAULTS
...
given.filename <- "5-Data-EpisodicDefaults-TruncationPoints"
...
p.D <- 0.5 # x% of accounts that should experience episodic default, p_D
...
for (it in 1:it.max) {
  ...
  # - Episode Length Probability Vector - specific to Episodic Defaults technique
  # Seed value - calculate first term of a finite geometric series (of length k) assuming common ratio r=0.5
  a <- 0.5*p.D/(1-0.5^(k))
  P.ED <- (sapply(1:(k), function(l,a){
    mult <- (0.5)^(l-1)
    return(a*mult)
  }, a=a))
  P.ED <- rep(1/(k),k) # uniform
  ...
  for (Loan in 1:n) {
    ...
    # determine if account should be NPL or PL
    p.del <- runif(1);
    if (p.del < p.D) { #non-performing account (NPL) - this account should experience episodic default
      
      # determine in which delinquency segment/bucket this account should be, according to given probabilities
      p.seg <- sample(x=1:(k), size= 1, replace=T, prob=P.ED);
      
      # determine possible starting times for this episode of default, given its sampled length (p.seg)
      statespace <- 1:(period-p.seg);
      
      # use uniform distributiom from which to sample a starting time for this account, in producing a probability vector
      P.Start <- rep(1/NROW(statespace),NROW(statespace));
      
      # sample starting time from this statespace of possible starting times from uniform distribution
      epi.start <- sample(x=statespace, size=1, replace=T, prob=P.Start);
      
      #counter for default episode's length
      epi.count <- 0;
      
      while (Month < period) {
        Month <- Month + 1;
        
        if (Month >= epi.start) { #sampled start point has been reached, default starts here
          
          if (epi.count < p.seg) { #current episode is still within sampled length, therefore default
            
            mat.Receipt[Month,Loan] <- 0;  
            epi.count <- epi.count + 1;
            
          } else { # episode has reached its zenith, therefore resume payment
            mat.Receipt[Month,Loan] <- vec.Instal[Loan];   
          }
          
        } else { # sampled start point has not yet been reached
          mat.Receipt[Month,Loan] <- vec.Instal[Loan];
        }
        
      }
      
    } else { #performing account (PL)
      mat.Receipt[,Loan] <- rep(vec.Instal[Loan],period)
    }
  }
