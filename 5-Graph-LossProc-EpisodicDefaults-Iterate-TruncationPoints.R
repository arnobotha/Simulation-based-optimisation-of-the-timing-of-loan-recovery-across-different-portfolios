...
# ======= Import Data
load("5-Data-EpisodicDefaults-TruncationPoints.RData")
...
prefixed <- "k="
col.headings <- sapply(1:it.max,function(i)paste(prefixed, formatC(it.vec[i],width=2,flag="0"),sep=''))
...
for (i in 1:it.max) {
  ...
  treat.data <- subset(plot.data2, Portfolio == paste(prefixed, formatC(it.vec[i],width=2,flag="0"),sep='') & Threshold > 0) 
  plot.data2 <- subset(plot.data2, !(Portfolio == paste(prefixed, formatC(it.vec[i],width=2,flag="0"),sep='') & Threshold > 0))
  sel.rows <- seq(1,nrow(treat.data),by=2)
  plot.data2 <- rbind(plot.data2, treat.data[sel.rows,])
  
  #remove points after reaching minimum
  foundMinima <- mat.d.CD[Position(function(x) x==min(mat.TotLoss.CD[,i]),mat.TotLoss.CD[,i]),i]
  plot.data4 <- subset(plot.data4, !(Portfolio == paste(prefixed, formatC(it.vec[i],width=2,flag="0"),sep='') & Threshold > foundMinima))
  
  plot.data3 <- rbind(plot.data3,
                      data.frame(Portfolio=paste(prefixed, formatC(it.vec[i],width=2,flag="0"),sep=''),
...
lgnd.label <- expression(Truncation~italic(k))
label.vec <- 1:it.max
...
plot.full <- 
  ...
  scale_shape_manual(values=1:it.max, name=lgnd.label, labels=label.vec) +
...
plot.zoom <- 
  ...
  geom_point(aes(x=Threshold,y=Loss, color=Portfolio, shape=Portfolio), data=plot.data3, size=3) +
  geom_point(aes(x=Threshold,y=Loss), data=plot.data3, size=7, colour="black", shape=1) +
  scale_x_continuous(breaks=pretty_breaks()) + 
  theme(legend.position="none", axis.text.x = element_text(margin=unit(c(0,0,0,-5),"mm"),size=9),
...
# -- merge plots
g <- plot.full + annotation_custom(grob = ggplotGrob(plot.zoom), xmin = 10, xmax=30,
                                   ymin = 0.1, ymax = 0.4)
g

dpi <- 200 #130 dpi used here for clarity in floating subplots in latex
ggsave(g, file="LossRateThresh_episodicdefaults_g1_k-g1-trunc.png", width=1200/dpi, height=1000/dpi,dpi=dpi)
