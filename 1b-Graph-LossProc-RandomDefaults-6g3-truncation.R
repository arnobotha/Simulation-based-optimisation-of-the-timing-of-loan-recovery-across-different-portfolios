...
# ======= Import Data
load("1b-Data-RandomDefaults-6g3.RData")
...
#CD
treat.data <- subset(plot.data2, Measure == "g1: CD" & Threshold > 0 )
...
#MD
treat.data <- subset(plot.data2, Measure == "g2: MD" & Threshold > 1.5 & Threshold <3.5)
plot.data2 <- subset(plot.data2, !(Measure == "g2: MD" & Threshold > 1.5 & Threshold < 3.5))
sel.rows <- seq(1,nrow(treat.data),by=2)
plot.data2 <- rbind(plot.data2, treat.data[sel.rows,])
treat.data <- subset(plot.data2, Measure == "g2: MD" & Threshold > 3.5)
plot.data2 <- subset(plot.data2, !(Measure == "g2: MD" & Threshold > 3.5))
sel.rows <- seq(1,nrow(treat.data),by=2)
plot.data2 <- rbind(plot.data2, treat.data[sel.rows,])
#DoD
treat.data <- subset(plot.data2, Measure == "g3: DoD" & Threshold > 3 & Threshold < 8)
plot.data2 <- subset(plot.data2, !(Measure == "g3: DoD" & (Threshold > 3 & Threshold < 8)))
sel.rows <- seq(1,nrow(treat.data),by=2)
plot.data2 <- rbind(plot.data2, treat.data[sel.rows,])
treat.data <- subset(plot.data2, Measure == "g3: DoD" & Threshold > 4)
plot.data2 <- subset(plot.data2, !(Measure == "g3: DoD" & (Threshold > 4 )))
sel.rows <- seq(1,nrow(treat.data),by=2)
plot.data2 <- rbind(plot.data2, treat.data[sel.rows,])
...
plot.full <- 
  ...
  scale_shape_manual(values=c(1,16,8), labels=label.vec,
                     name="Delinquency Measure",guide = guide_legend(ncol=3)) +
...
# -- zoomed plot
# zoom bounding box
xlim <- c(0,13); ylim <- c(0.23, 0.255) #for random defaults
#xlim <- c(0,13); ylim <- c(8500000, 9600000) #for Markovian defaults

plot.zoom <- 
  plot.full + coord_cartesian(xlim=xlim, ylim=ylim) + 
  geom_point(aes(x=Threshold,y=Loss, color=Measure, shape=Measure), data=plot.data, size=2) +
  ...
  theme(legend.position="none", axis.line=element_blank(),axis.text.x = element_text(margin=unit(c(0,0,0,0),"mm"),size=9),
...
# -- merge plots
g <- plot.full + annotation_custom(grob = ggplotGrob(plot.zoom), xmin = 12, xmax=40,
                                   ymin = 0.3, ymax = 0.4)
g


dpi <- 300 
ggsave(g, file="LossRateThresh_randomdefaults_6g3-trunc.png", width=1200/dpi, height=1000/dpi,dpi=dpi)
